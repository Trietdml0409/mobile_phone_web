...
function CardComponent({person,handleClickName,clickedName}:{person:Person;handleClickName(name:string):void;clickedName:(string|null)}){
    return(
        <Card>
            <img src={person.image}/>
            <p>{person.name}</p>
            <p>{person.age}</p>
            <Button style={{...(clickedName === person.name ? {background:"blue",color:"white"}:{})}}onClick = {() => {handleClickName(person.name)}}>Click here</Button>
        </Card>
            ...
            <div style={{backgroundColor:"white", height:"100vh",width:"100vw"}}>
                ...
                <Flex gap="middle">
                    {persons.map((person:Person)=>(
                        <CardComponent clickedName={clickedName} handleClickName={setClickedName} key={person.id} person={person} />
                    ))}
                </Flex>
            </div>
        </>
